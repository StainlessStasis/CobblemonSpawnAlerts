...
* **{level}**: Inserts the Pokemon's level if `showLevel` is enabled
* **{ivs}**: Inserts the Pokemon's IVs if `showIVs` is enabled
* **{nature}**: Inserts the Pokemon's nature if `showNature` is enabled
* **{coords}**: Inserts the Pokemon's coordinates if `showCoordinates` is enabled
<br><br>
### Examples:<br>
...
* Option to have alerts for ANY shiny or legendary (using the default templates)
...
* Sounds
* Alerts for Pokemon that you don't have in the dex/haven't caught yet
